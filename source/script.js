...
Canvas.prototype.drawLines = function (particle, color) {
  ...
  for (let index = 0; index < particle.closest.length; index++){
...
function Particle(options, index) {
  ...
  // Speed
  this.speed = particleReferenceSpeed * getRandom(0.5,2);

  // Size
  this.size = particleReferenceSize * getRandom(1,2);
...
Particle.prototype.update = function(target, index) { 
    // Man könnte die Geschwindigkeit in Abhängigkeit zum Abstand zwischen target und this.position berechnen.
  ...
  // variable Größe macht das ganze doch nochmal echter.
  // if(!isSafari) {
...
const particleReferenceSpeed = 0.02;   // percentage speed of particles
const particleReferenceSize = 2;       // Sizer of particles in pixels.
const particleReferenceOrbitSize = 10; // References orbit size means a referential distance to the center (mouse cursor) position. In pixel.
const maximumLinkDistances = 100;      // maximum length of connection lines between particles in pixels.
